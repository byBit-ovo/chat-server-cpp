...
MY_IM::ServiceChannels::ChannelPtr channel;
std::string single_file_id;

TEST(put_test, single_file) {
    //1. 读取当前目录下的指定文件数据
    std::string body;
    ASSERT_TRUE(MY_IM::ReadFile("./Makefile", body));
    ...
    MY_IM::PutSingleFileRsp *rsp = new MY_IM::PutSingleFileRsp();
...
TEST(get_test, single_file) {
    ...
    brpc::Controller *cntl = new brpc::Controller();
    rsp = new MY_IM::GetSingleFileRsp();
    stub.GetSingleFile(cntl, &req, rsp, nullptr);
    ASSERT_FALSE(cntl->Failed());
    ASSERT_TRUE(rsp->success());
    //将文件数据，存储到文件中
    ASSERT_EQ(single_file_id, rsp->file_data().file_id());
    ASSERT_TRUE(MY_IM::WriteFile("./recieve/make_file_download", rsp->file_data().file_content()));
}
...
TEST(put_test, multi_file) {
    //1. 读取当前目录下的指定文件数据
    std::string body1;
    ASSERT_TRUE(MY_IM::ReadFile("./base.pb.h", body1));
    std::string body2;
    ASSERT_TRUE(MY_IM::ReadFile("./file.pb.h", body2));
    //2. 实例化rpc调用客户端对象，发起rpc调用
    MY_IM::FileService_Stub stub(channel.get());
    ...
    brpc::Controller *cntl = new brpc::Controller();
    MY_IM::PutMultiFileRsp *rsp = new MY_IM::PutMultiFileRsp();
...
TEST(get_test, multi_file) {
	...
	ASSERT_TRUE(MY_IM::WriteFile("./recieve/file_download_file2", file_data2.file_content()));
	ASSERT_TRUE(MY_IM::WriteFile("./recieve/base_download_file1",file_data1.file_content()));
}
...
int main(int argc, char *argv[])
{
    ...
    //1. 先构造Rpc信道管理对象
    auto sm = std::make_shared<MY_IM::ServiceManager>();
    sm->FollowOn(FLAGS_file_service);
    auto put_cb = std::bind(&MY_IM::ServiceManager::OnlineCall, sm.get(), std::placeholders::_1, std::placeholders::_2);
    auto del_cb = std::bind(&MY_IM::ServiceManager::OfflineCall, sm.get(), std::placeholders::_1, std::placeholders::_2);
    //2. 构造服务发现对象
