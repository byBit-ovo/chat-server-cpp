...
int main(int argc, char *argv[])
{
    ...
    sm->FollowOn(FLAGS_speech_service);
	...
	LOG_INFO("File length:{}",file_content.size());
    //4. 发起EchoRpc调用
    MY_IM::SpeechService_Stub stub(channel.get());
    MY_IM::SpeechRecognitionReq req;
    req.set_speech_content(file_content);
    req.set_request_id("111111");

    brpc::Controller *cntl = new brpc::Controller();
    MY_IM::SpeechRecognitionRsp *rsp = new MY_IM::SpeechRecognitionRsp();
	// Rpc call
