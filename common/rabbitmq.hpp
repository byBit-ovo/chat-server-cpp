        ...
        void DeclareComponents(const std::string &exchange,
            ...
            _channel->declareExchange(exchange, echange_type)
                .onError([](const char *message) {
                    LOG_ERROR("Declare Exchange fail!{}", message);
                    exit(0);
                })
                .onSuccess([exchange](){
                    LOG_ERROR("{} Exchange create successfully!", exchange);
                });
            _channel->declareQueue(queue)
                .onError([](const char *message) {
                    LOG_ERROR("Declare queue fail!{}", message);
                    exit(0);
                })
                .onSuccess([queue](){
                    LOG_ERROR("queue create successfully!", queue);
                });
            //6. 针对交换机和队列进行绑定
            _channel->bindQueue(exchange, queue, routing_key)
                .onError([exchange, queue](const char *message) {
                    LOG_ERROR("{} - {} Bind fail!:", exchange, queue);
                    exit(0);
                })
                .onSuccess([exchange, queue, routing_key](){
                    LOG_ERROR("{} - {} - {} Bind successfully!", exchange, queue, routing_key);
                });
        }
        bool Publish(const std::string &exchange, 
            const std::string &msg, 
            const std::string &routing_key = "routing_key") {
            LOG_DEBUG("Publish to {}(Exchange)-{}(Routing_Key) !", exchange, routing_key);
            bool ret = _channel->publish(exchange, routing_key, msg);
            if (ret == false) {
                LOG_ERROR("{} Publish fail:", exchange);
                return false;
            }
            return true;
        }
        void Consume(const std::string &queue, const MessageCallback &cb) {
            LOG_DEBUG("Subscribe {}(Queue-Message)!", queue);
